...
make_rmd <- function(listobject, n = 10) {
  ...
  if (
    listobject$indicatorname %in%
      c("bottom_temp_model_gridded", "thermal_habitat_gridded", "ches_bay_sst")
  ) {
  ...
  ### PAGE TITLE
  cat(
    paste0("# ", listobject$dataname, " {#", listobject$indicatorname, "}"),
    append = T,
    fill = T,
    file = con
  )
  ...
  ### ISSUE LAST MODIFIED
  cat(
    paste0("*Last updated ", listobject$last_updated, "*"),
    append = T,
    fill = T,
    file = con
  )
  ...
  cat(
    paste0("**Description**: ", listobject$description),
    append = T,
    fill = T,
    file = con
  )
  cat("", append = T, fill = T, file = con) # add space
  cat(
    paste0("**Contributor(s)**: ", listobject$contributors),
    append = T,
    fill = T,
    file = con
  )
  cat("", append = T, fill = T, file = con) # add space
  cat(
    paste0("**Affiliations**: ", listobject$affiliations),
    append = T,
    fill = T,
    file = con
  )
  ...
  if (page_type == "ecodata" & page_subtype != "spatial") {
    ...
    cat(
      paste0("```{r download_", listobject$indicatorname, "}"),
      append = T,
      fill = T,
      file = con
    )
    ...
    cat(
      paste0("ecodata::", listobject$indicatorname, ","),
      append = T,
      fill = T,
      file = con
    )
    ...
    cat(
      paste0("output_name = '", listobject$indicatorname, "',"),
      append = T,
      fill = T,
      file = con
    )
  ...
  if (exists(paste0("plot_", listobject$indicatorname))) {
    ...
    plot_code_strings <- ecodata::create_all_plots(
      ecodata_name = listobject$indicatorname,
      write_only = T,
      n = 10
    )
    ...
    for (i in 1:length(plot_code_strings)) {
      ...
      # Open and name code chunk
      cat(
        paste0("```{r plot_", listobject$indicatorname, chunk_name, "}"),
        append = T,
        fill = T,
        file = con
      )
      ...
      cat(
        paste0("ggplotObject <- ", plot_code_strings[i], " +"),
        append = T,
        fill = T,
        file = con
      )
      ...
      if (page_subtype != "spatial") {
        ...
        ## Name output file using ecodata dataset name
        cat(
          paste0("output_name = '", listobject$indicatorname, chunk_name, "',"),
          append = T,
          fill = T,
          file = con
        )
  ...
  cat(
    paste0("Spatial scale: ", listobject$indicatorStatsSpatial),
    append = T,
    fill = T,
    file = con
  )
  cat("", append = T, fill = T, file = con) # add space
  cat(
    paste0("Temporal scale: ", listobject$indicatorStatsTemporal),
    append = T,
    fill = T,
    file = con
  )
  ...
  if (page_type == "independent") {
    cat(
      unlist(strsplit(listobject$defineVariables, "\\n")),
      append = T,
      fill = T,
      file = con
    )
  ...
  if (page_type == "ecodata") {
    ...
    if (
      "Var" %in%
        names(eval(parse(text = paste0("ecodata::", listobject$indicatorname))))
    ) {
      # r code chunk to make table
      cat(
        paste0("```{r vars_", listobject$indicatorname, "}"),
        append = T,
        fill = T,
        file = con
      )
      ...
      cat(
        paste0("vars <- ecodata::", listobject$indicatorname, " |>"),
        append = T,
        fill = T,
        file = con
      )
      if (
        eval(parse(
          text = paste0(
            "'Units' %in% names(ecodata::",
            listobject$indicatorname,
            ")"
          )
        ))
      ) {
  ...
  ### CONTACT PERSON
  cat(
    paste0("**Point of contact**: ", listobject$poc),
    append = T,
    fill = T,
    file = con
  )
  ...
  if (page_type == "ecodata") {
    ...
    cat(
      paste0("**ecodata dataset**: `ecodata::", listobject$indicatorname, "`"),
      append = T,
      fill = T,
      file = con
    )
    ...
    cat(
      paste0(
        "**Tech Doc link**: <https://noaa-edab.github.io/tech-doc/",
        listobject$indicatorname,
        ".html>"
      ),
      append = T,
  ...
  if (
    listobject$publicAvailability == "Source data are NOT publicly available."
  ) {
